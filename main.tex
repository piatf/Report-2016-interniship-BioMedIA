...
mirtk extract-image-region input.nii.gz output.nii.gz -Rx1 50 -Rx2 40 -Ry1 250 -Ry2 250
	...
	Le dossier \textit{Applications} du MIRTK regroupe les fichiers correspondants aux commandes exécutables. Ils font appel à certaines dépendances, qui sont réparties en 7 modules : \textit{Common, Image, I/O, Numerics, PointSet, Registration} et \textit{Transformation}. Ces différents modules représentent le lien entre le back-end du MIRTK et son corps. Le recalage d'image (Registration) est l'une de ses principales fonctions.
...
\chapter{Objectifs et cahier des charges}
	...
	\\{$\bullet$}\textbf{ un réseau local}, %pas un reseau, une machine 
	qui distribuera les tâches sur des machines de caractéristiques différentes au niveau du hardware. Chaque machine pourra exécuter ce qu'elle reçoit sur un CPU, un GPU, ou plusieurs GPU (multi-GPU).
	...
	\\{$\bullet$}\textbf{ une grille de calculs}, %ex: euro(cern) / hpc (cf imperial)
	 qui s'étend à tout le réseau de machines de l'Imperial college, mais qui est un service payant.
...
%	Le MIRTK bénéficie seulement du multi-threading (malgré toutes les solutions précédentes), mais, bien que les opérations de traitement d'image utilisent souvent le GPU d'une machine n'est pas capable d'utiliser d'autres moyens d'accélération matériels comme l'utilisation d'un GPU.
%	\\
	...
	\\{$\bullet$} La compatibilité avec différents back-end, que ce soit pour CPU ou GPU. A l'heure actuelle, la gestion des back-end GPU n'est pas assurée par le logiciel. Le rendu idéal serait de laisser le MIRTK gérer automatiquement cet aspect, en vérifiant la présence d'une carte graphique sur l'ordinateur (sinon utiliser le CPU), puis le laisser choisir entre CPU et GPU pour des performances optimales. Si, par exemple, un ordinateur possède une bonne carte graphique, le MIRTK devra être apte à le détecter, et donc de l'utiliser.\\
	\\{$\bullet$} Garder l'intégrité et la transparence du code du MIRTK. Il sera imposé de garder les mêmes fonctions (ainsi que leurs arguments) et la même API. Pour conserver cela, il faudra garder une transparence entre le code du MIRTK et la gestion de différents back-end. C'est-à-dire que l'implémentation la plus haut niveau des fonctions du logiciel devra rester inchangée.\\
	...
	~\par~\par 
	1) \textbf{Études du MIRTK et de ses dépendances, et analyse de l'API d'ArrayFire} \\
	Cette étape préliminaire permettra de comprendre le fonctionnement global du MIRTK et de se familiariser avec son code source. Dans le même temps, l'analyse de l'interface d'ArrayFire, et des différentes fonctions proposées par la bibliothèque. Cette étude corrélera donc les besoins du MIRTK et les solutions proposées par ArrayFire.
	~\par~\par 
	...
	Afin d'apporter une fonctionnalité davantage liée au hardware utilisé, la gestion des back-ends serait un avantage pour les performances du MIRTK. Pour cette raison, cette gestion sera idéalement implémentée en fin de stage. 
	~\par~\par 
	\t 5) \textbf{Parangonage} \\
